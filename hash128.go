...
func HashString128(s string) [2]uint64 {
	fn := hashMed128
	if len(s) <= 16 {
		fn = hashSmall128
	}
...
func hashSmall128(p ptr, l int) (acc [2]u64) {
	switch {
	case l > 8:
		...
		acc[0], acc[1] = bits.Mul64(m128_l, prime64_2)
		acc[0] += m128_h * prime64_2
	...
	case l >= 4:
		...
		acc[0], acc[1] = bits.Mul64(keyed, prime64_1+(uint64(l)<<2))

		acc[0] += acc[1] << 1
		acc[1] ^= acc[0] >> 3

		acc[1] ^= acc[1] >> 35
		acc[1] *= 0x9fb21c651e98df25
		acc[1] ^= acc[1] >> 28
		acc[0] = xxh3Avalanche(acc[0])
	...
	case l == 2:
		c12 := u64(readU16(p, 0))
		acc[1] = c12*(1<<24+1)>>8 + 2<<8

	case l == 1:
		c1 := u64(readU8(p, 0))
		acc[1] = c1*(1<<24+1<<16+1) + 1<<8

	case l == 0:
		return [2]uint64{0x99aa06d3014798d8, 0x6001c324468d497f}
	}

	acc[0] = uint64(bits.RotateLeft32(bits.ReverseBytes32(uint32(acc[1])), 13))
	acc[1] ^= uint64(key32_000 ^ key32_004)
	acc[0] ^= uint64(key32_008 ^ key32_012)

	acc[1] = xxh64AvalancheSmall(acc[1])
	acc[0] = xxh64AvalancheSmall(acc[0])

	return acc
}

func hashMed128(p ptr, l int) (acc [2]u64) {
	const seed = 0

	switch {
	case l <= 128:
		acc[1] = u64(l) * prime64_1
		...
		if l > 32 {
			if l > 64 {
				if l > 96 {
					...
					acc[0] += mulFold64(in8^key64_112, in7^key64_120)
					acc[0] ^= i6 + i7

					acc[1] += mulFold64(i6^key64_096, i7^key64_104)
					acc[1] ^= in8 + in7
				...
				acc[0] += mulFold64(in6^key64_080, in5^key64_088)
				acc[0] ^= i4 + i5

				acc[1] += mulFold64(i4^key64_064, i5^key64_072)
				acc[1] ^= in6 + in5
			...
			acc[0] += mulFold64(in4^key64_048, in3^key64_056)
			acc[0] ^= i2 + i3

			acc[1] += mulFold64(i2^key64_032, i3^key64_040)
			acc[1] ^= in4 + in3
		...
		acc[0] += mulFold64(in2^key64_016, in1^key64_024)
		acc[0] ^= i0 + i1

		acc[1] += mulFold64(i0^key64_000, i1^key64_008)
		acc[1] ^= in2 + in1

		acc[0], acc[1] = (acc[1]*prime64_1)+(acc[0]*prime64_4)+(u64(l)*prime64_2), acc[0]+acc[1]
	...
	case l <= 240:
...

		{
			...
			acc[0] += mulFold64(i2^key64_016, i3^key64_024)
			acc[0] ^= i0 + i1
			acc[1] += mulFold64(i0^key64_000, i1^key64_008)
			acc[1] ^= i2 + i3
		}

		{
			...
			acc[0] += mulFold64(i2^key64_048, i3^key64_056)
			acc[0] ^= i0 + i1
			acc[1] += mulFold64(i0^key64_032, i1^key64_040)
			acc[1] ^= i2 + i3
		}

		{
			...
			acc[0] += mulFold64(i2^key64_080, i3^key64_088)
			acc[0] ^= i0 + i1
			acc[1] += mulFold64(i0^key64_064, i1^key64_072)
			acc[1] ^= i2 + i3
		}

		{
			...
			acc[0] += mulFold64(i2^key64_112, i3^key64_120)
			acc[0] ^= i0 + i1
			acc[1] += mulFold64(i0^key64_096, i1^key64_104)
			acc[1] ^= i2 + i3
		}
		...
		for i := ui(4 * 32); i < top; i += 32 {
			...
			acc[0] += mulFold64(i2^k2, i3^k3)
			acc[0] ^= i0 + i1
			acc[1] += mulFold64(i0^k0, i1^k1)
			acc[1] ^= i2 + i3
		}
		...
		// last 32 bytes
		{
			...
			acc[0] += mulFold64(i0^key64_119, i1^key64_127)
			acc[0] ^= i2 + i3
			acc[1] += mulFold64(i2^key64_103, i3^key64_111)
			acc[1] ^= i0 + i1
		}
